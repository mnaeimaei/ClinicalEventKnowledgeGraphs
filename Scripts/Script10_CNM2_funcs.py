...
def Create_CSV_in_Neo4J_import10(union):
    sampleIds = []
...
def CreateRel(csvLog_header_Diag, icd_code,  SCT_ID):
    dataF=csvLog_header_Diag
    column_names = [icd_code, SCT_ID]

    #print(column_names)

    # Get distinct values from the specified columns
    distinct_values = dataF[column_names].drop_duplicates().values.tolist()
    converted_NodeList = [[item[0], item[1]] for item in distinct_values]
...
def Potential_OCPS(tx, icd_code, SCTID):
