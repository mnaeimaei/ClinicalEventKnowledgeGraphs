...
def Create_CSV_in_Neo4J_import9(union):
    sampleIds = []
...
def CreateRel(csvLog_header_Diag, Disorders_id, icd_code):
    dataF=csvLog_header_Diag
    column_names = [Disorders_id, icd_code]
    ...
    converted_NodeList = [[item[0], str(item[1])] for item in distinct_values]
...
def Entity1_Potential_Entities(tx, ID, icdCode):
    qLinkSCTs = f''' 
